---
hide:
  - navigation
---
# Programació didàctica

La [programació didàctica completa del curs 2026–2027](programacio-didactica-26-27.pdf) es conserva en PDF per a consulta del professorat i de l’alumnat.

## Dades bàsiques

- **Mòdul:** Aplicacions Web.
- **Codi:** 0228.
- **Cicle:** Sistemes Microinformàtics i Xarxes.
- **Curs:** 2n.
- **Durada:** 100 hores lectives.
- **Distribució:** 3 hores setmanals.
- **Formació:** 73 hores al centre i 27 hores en empresa.

## Unitats i càrrega al centre

| UP | Resultat d’aprenentatge | Hores | Pes |
| --- | --- | ---: | ---: |
| UP1 | Gestors de continguts · RA1 | 22 | 29 % |
| UP2 | Sistemes de gestió d’aprenentatge a distància · RA2 | 19 | 26 % |
| UP3 | Serveis de gestió d’arxius web · RA3 | 20 | 27 % |
| UP4 | Aplicacions d’ofimàtica web · RA4 | 6 | 9 % |
| UP5 | Aplicacions web d’escriptori · RA5 | 6 | 9 % |
| **Total** |  | **73** | **100 %** |

La formació en empresa completa les 27 hores restants i inclou criteris d’avaluació observables en l’entorn laboral.

## Resultats d’aprenentatge

1. Instal·lar gestors de continguts, identificar les seues aplicacions i configurar-los segons requisits.
2. Instal·lar sistemes de gestió d’aprenentatge a distància, descrivint l’estructura del lloc i la jerarquia de directoris generada.
3. Instal·lar serveis de gestió d’arxius web, identificar les seues aplicacions i verificar-ne la integritat.
4. Instal·lar aplicacions d’ofimàtica web, descrivint les seues característiques i entorns d’ús.
5. Instal·lar aplicacions web d’escriptori, descrivint les seues característiques i entorns d’ús.

Per a la informació pràctica de l’alumnat, consulta la [guia de l’assignatura](guia-assignatura.md) i la pàgina d’[avaluació](avaluacio.md).